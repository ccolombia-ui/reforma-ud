...
> Inventario completo con archivos en `99--sources/biblio/` (62 fichas APA + BibTeX). Ver `_Index_of_99--sources.md` para navegación.
...
**Receta NORMATIVO** (Facultad/Escuela/CABA): ver [`con-escuela`](/glosario/con-escuela/) — frontmatter de referencia con `concepto_capabilities: [NORMATIVE, DDD]`, facets `concepto_facet_normative` (norm_legal_ref, norm_article, norm_jurisdiction, norm_effective_date, norm_legal_force) + `concepto_facet_ddd` (ddd_aggregate_root, ddd_invariants, ddd_ubiquitous_terms).

**Receta ACADÉMICO** (CABA con triple anclaje): ver [`con-caba`](/glosario/con-caba/) v1.1.0 — `concepto_capabilities: [NORMATIVE, DDD, NEON]`, facet NeOn S5 (merging: Wenger + ACU + Geels).

**Receta DATO-CALCULADO con norma**: ver [`con-credito-academico`](/glosario/con-credito-academico/) — caso raro `[NORMATIVE, EQUATION]` con fórmula vinculante `h_total = 48·n_cr` declarada en Decreto 1330 Art. 11.
...
- El **átomo NORMATIVO canónico** vive en [[con-escuela|`con-escuela`]] — definición vinculante única según ACU-004-25 Art. 58-72. **SSOT**.
