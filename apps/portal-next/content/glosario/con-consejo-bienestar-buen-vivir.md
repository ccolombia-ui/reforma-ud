---
kd_id: urn:aleia:udfjc:reforma:cap-mi12:con:consejo-bienestar-buen-vivir
kd_title: "Consejo de Bienestar y Buen Vivir UDFJC (Art. 18l ACU-004-25)"
kd_type: glosario-universal
kd_status: APPROVED
kd_version: v1.0.0

tupla_tipo: CONCEPT
tupla_concepto: "Consejo de Bienestar y Buen Vivir UDFJC"
tupla_descripcion: "Órgano de gobierno colegiado del Sistema de Bienestar y Buen Vivir UDFJC declarado en Art. 18l · articula representación multi-estamento + dimensión decolonial"
tupla__container_jsonl: "00-glosoario-universal/conceptos-normativos.jsonl"

skos_prefLabel: "Consejo de Bienestar y Buen Vivir"
skos_altLabel: ["CBBV", "Wellbeing Council"]

skos_definition: "Órgano de gobierno colegiado del Sistema de Bienestar y Buen Vivir UDFJC declarado como **órgano del Gobierno Universitario** en el Art. 18l del ACU-004-25. Articula la deliberación sobre políticas integradas de bienestar (servicios tradicionales) + Buen Vivir (dimensión decolonial andina) con representación multi-estamento (4 estamentos de la Comunidad Universitaria). Su rol es **deliberativo-propositivo** sobre el Sistema de Bienestar y Buen Vivir (Arts. 88-90) operado por la Dirección de Bienestar Universitario y Buen Vivir (DBBV). Su composición específica se desarrollará en el Estatuto de Bienestar y Buen Vivir nuevo (Art. 98 §7)."
skos_scopeNote: "El CBBV es DIFERENTE del Consejo de Gestión Administrativa (CGA · Art. 87): el CGA articula coordinación administrativa al nivel de directivos, el CBBV es órgano de participación democrática multi-estamento sobre temas de bienestar. La DBBV (Dirección) es la dependencia ejecutiva — el CBBV es el órgano deliberativo. Sin el CBBV, el Sistema Bienestar y Buen Vivir operaría sin instancia colegiada de participación de la Comunidad."
skos_example: "Cuando UDFJC debe decidir políticas de soberanía alimentaria con cafeterías saludables + huerta universitaria + saberes ancestrales, el proceso integra deliberación del CBBV (multi-estamento) + ejecución de la DBBV (operacional con asiento en CGA)."
skos_notation: "CBBV"

iso_designation_type: term
iso_definition_type: extensional
iso_genus: "Órgano de gobierno colegiado universitario sobre bienestar y principios refundacionales"
iso_differentia: "Deliberación multi-estamento sobre Sistema Bienestar y Buen Vivir · diferente del CGA (administrativo) y de la DBBV (ejecutiva) · órgano declarado Art. 18l"
iso_subject_field: "Gobernanza democrática universitaria · Bienestar institucional · Principios decoloniales"
iso_term_status: preferred
iso_standardized_by: "Acuerdo CSU UDFJC 04/2025 Art. 18l + Arts. 88-90"

pasteur_quadrant: PASTEUR

concepto_capabilities:
  - NORMATIVE
  - DDD

concepto_facet_normative:
  origin_type: INSTITUTIONAL_BINDING
  origin_source: "[[cita-acu-004-25-csu-udfjc-2025]]"
  origin_force: BINDING
  adoption_chain:
    - adopter: "[[con-acu-004-25]]"
      adopter_locator: "ACU-004-25 Art. 18l + Arts. 88-90"
      adopter_authority_level: ESTATUTARIO
      adopted_at: "2025-05-05"
      adoption_evidence: "ACU-004-25 declara Consejo de Bienestar y Buen Vivir como órgano del Gobierno Universitario · concepto NUEVO sin precedente en ACU 003/1997"
  effective_force_in_udfjc: BINDING_DIRECT
  effective_authority_level: ESTATUTARIO
  normative_source: "[[cita-acu-004-25-csu-udfjc-2025]]"
  normative_locator: "ACU-004-25 Art. 18l + Arts. 88-90"
  normative_text: "Órganos del Gobierno: l) El Consejo de Bienestar y Buen Vivir (Art. 18l)."
  normative_authority_level: ESTATUTARIO
  derogated_by: ""
  derogates: ["Concepto NUEVO en ACU-004-25 (sin precedente exacto en ACU 003/1997)"]
  modification_type: ""
  chain_status: LINEAR
  conflicts_with: []

concepto_facet_ddd:
  ddd_id: "consejo_bienestar_buen_vivir"
  ddd_term: "Consejo de Bienestar y Buen Vivir"
  ddd_aggregate_root: "ConsejoBienestarBuenVivir"
  ddd_article_ref: "Art. 18l + Arts. 88-90"
  ddd_source_norm: "[[cita-acu-004-25-csu-udfjc-2025]]"
  ddd_bc_ref: "[[bc-bienestar-buen-vivir-udfjc]]"
  ddd_role_in_context: AGGREGATE_ROOT
  ddd_aggregate_root_flag: true
  ddd_domain_type: Core
  ddd_governed_by:
    - "[[con-direccion-bienestar-buen-vivir]]"
  ddd_invariants:
    - "CBBV es órgano deliberativo · NO ejecutivo (esa función es DBBV)"
    - "CBBV diferente del CGA: CGA es administrativo de directivos · CBBV es democrático multi-estamento"
    - "Composición específica desarrollada en Estatuto Bienestar Art. 98 §7"
    - "Articula bienestar tradicional + Buen Vivir decolonial como dimensiones complementarias"
  ddd_ubiquitous_terms:
    - "CBBV"
    - "Consejo deliberativo multi-estamento"
    - "Bienestar y Buen Vivir"

concepto_definitional_anchors: ["[[def-norm-acu-004-25-art-18l-2025-05-05]]"]
concepto_current_anchor: "[[def-norm-acu-004-25-art-18l-2025-05-05]]"
concepto_anchor_chain_status: LINEAR

concepto_prerequisitos:
  - "[[con-acu-004-25]]"
  - "[[con-buen-vivir]]"
  - "[[con-sistema-bienestar-buen-vivir]]"
  - "[[con-direccion-bienestar-buen-vivir]]"

applicable_domain: "Toda deliberación sobre bienestar y buen vivir UDFJC desde 2025-05-06"
assumptions: ["Composición específica en Estatuto Bienestar Art. 98 §7"]
breaks_at: ["Si CBBV se confunde con CGA o DBBV (separación de funciones)"]

valid_from: "2025-05-06"
valid_to: ""
rol_seleccionado: estudiante-soberano


tupla__relations:
  - rel_id: rel-cbbv-implements-acu
    rel_nombre: norm_implements
    rel_direccion: pre
    rel_target: "[[con-acu-004-25]]"
    rel_frame: normativo
    rel_propiedades:
      norm_evidence: "CBBV declarado Art. 18l del ACU-004-25 como órgano del Gobierno Universitario"
  - rel_id: rel-cbbv-articula-dbbv
    rel_nombre: skos_related
    rel_direccion: co
    rel_target: "[[con-direccion-bienestar-buen-vivir]]"
    rel_frame: skos
    rel_propiedades:
      skos_evidence: "CBBV delibera · DBBV ejecuta · ambos sobre Sistema Bienestar y Buen Vivir"
  - rel_id: rel-cbbv-articula-sistema
    rel_nombre: ddd_part_of
    rel_direccion: post
    rel_target: "[[con-sistema-bienestar-buen-vivir]]"
    rel_frame: ddd
    rel_propiedades:
      norm_evidence: "CBBV es órgano deliberativo del Sistema Bienestar y Buen Vivir Arts. 88-90"

cited_in: ["[[sec-MI12-00--carta-constitucional-acu-004-25]]"]
cited_count: 1

tags: [glosario-universal, concepto-normativo, t1-normativo, consejo-bienestar, cbbv, art-18l, organo-gobierno, m00-base, audit-v2-2, tpl-v2, cop-fundacional]
---


# Consejo de Bienestar y Buen Vivir

> [!note]+ Órgano colegiado deliberativo · multi-estamento
> El **CBBV** delibera sobre el Sistema de Bienestar y Buen Vivir · diferente de la DBBV (ejecutiva) y del CGA (administrativo). Concepto NUEVO sin precedente en ACU 003/1997.

## §0 · 🎭 Vista por rol

<span class="dv-block" data-dv="selector-rol"></span>

## §1 · Definición

> Órgano de gobierno colegiado del Sistema de Bienestar y Buen Vivir UDFJC declarado como **órgano del Gobierno Universitario** en el Art. 18l del ACU-004-25. Articula la deliberación sobre políticas integradas de bienestar (servicios tradicionales) + Buen Vivir (dimensión decolonial andina) con representación multi-estamento (4 estamentos de la Comunidad Universitaria). Su rol es **deliberativo-propositivo** sobre el Sistema de Bienestar y Buen Vivir (Arts. 88-90) operado por la Dirección de Bien

## §2 · Anclaje


<div class="dv-block" data-dv="facet-normative"></div>


## §3 · DDD


<div class="dv-block" data-dv="facet-ddd"></div>


## §7 · Relaciones


<div class="dv-block" data-dv="relations"></div>


## §10 · Citado en


<div class="dv-block" data-dv="cited-in"></div>


---

| Versión | Fecha | Cambios |
|---|---|---|
| **v1.0.0** | **2026-04-27** | Sprint 1A.6 Grupo F. CBBV concepto NUEVO en ACU-004-25. |

*CC BY-SA 4.0 · UDFJC · 2026-04-27 · `con-consejo-bienestar-buen-vivir` v1.0.0*
